...
export class FormValidator extends EventEmitter<FormValidatorEvents> {
  public readonly formElement: HTMLFormElement;
  public readonly schema: FormSchema<any>;
  ...
  constructor(
    ...
    this.stateManager = new FormStateManager(schema.getFieldNames() as string[]);
    this._initialize();
  }
  ...
  private _setupFieldListeners(): void {
    this.schema.getFieldNames().forEach(fieldName => {
      const inputElement = this.formElement.querySelector<HTMLInputElement>(`[name="${String(fieldName)}"]`);
      if (inputElement) {
        const handler = (event: Event) => this._handleFieldInteraction(event.type, fieldName as string);
  ...
  private _handleFieldInteraction(eventType: string, fieldName: string): void {
    const shouldDebounce = eventType === 'input' && this.options.debounceDelay > 0;
  ...
  public validateField(fieldName: string, showError: boolean): { isValid: boolean, error: string | null } {
    const inputElement = this.formElement.querySelector<HTMLInputElement>(`[name="${fieldName}"]`);
  ...
  private _showFieldError(fieldName: string, errorMessage: string): void {
    ...
    const errorElement = this.formElement.querySelector(`[${this.options.errorAttribute}="${fieldName}"]`);
  ...
  public handleSubmit(): void {
    ...
    this.schema.getFieldNames().forEach(fieldName => this._clearFieldError(fieldName as string));
  ...
  private _forwardStateEvents(): void {
    this.stateManager.on('fieldStateChange', (data) => this.emit('fieldStateChange', data));
    this.stateManager.on('formStateChange', (data) => this.emit('formStateChange', data));
  }
